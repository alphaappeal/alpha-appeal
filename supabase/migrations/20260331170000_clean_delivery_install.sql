-- =====================================================
-- DELIVERY MANAGEMENT SYSTEM - CLEAN INSTALL
-- Purpose: Creates ONLY missing delivery tables/functions
...
-- =====================================================
-- PART 1: CHECK AND CREATE TABLES IF NOT EXISTS
-- =====================================================
...
-- Insert default providers (safe re-execution)
...
-- =====================================================
-- PART 2: ENHANCE USER_DELIVERIES TABLE
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_deliveries' AND column_name = 'assignment_method') THEN
    ALTER TABLE public.user_deliveries ADD COLUMN assignment_method TEXT DEFAULT 'automatic';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_deliveries' AND column_name = 'traffic_conditions') THEN
    ALTER TABLE public.user_deliveries ADD COLUMN traffic_conditions TEXT DEFAULT 'normal';
  END IF;
END $$;
...
-- =====================================================
-- PART 3: DRIVER MANAGEMENT TABLES
-- =====================================================
...
-- =====================================================
-- PART 4: DELIVERY ASSIGNMENTS TABLE
-- =====================================================
...
BEGIN
  ...
  IF _order_weight_kg > 5 THEN
    v_final_fee := v_final_fee + ((_order_weight_kg - 5) * 5);
  END IF;
...
DROP POLICY IF EXISTS "Anyone can view active providers" ON public.delivery_service_providers;
DROP POLICY IF EXISTS "Admins can manage providers" ON public.delivery_service_providers;
DROP POLICY IF EXISTS "Drivers can view own profile" ON public.delivery_drivers;
DROP POLICY IF EXISTS "Vendors can view their drivers" ON public.delivery_drivers;
...
DROP POLICY IF EXISTS "Vendors can view assignments for their store" ON public.delivery_assignments;
DROP POLICY IF EXISTS "Admins can view all assignments" ON public.delivery_assignments;
DROP POLICY IF EXISTS "Vendors can view their zones" ON public.delivery_zones;
DROP POLICY IF EXISTS "Vendors can manage their zones" ON public.delivery_zones;
DROP POLICY IF EXISTS "Vendors can view their pricing" ON public.delivery_pricing;
DROP POLICY IF EXISTS "Vendors can manage their pricing" ON public.delivery_pricing;
